...
typedef unsigned int D_STAR_MAP_DIST_TYPE;
typedef Map2D<D_STAR_MAP_DIST_TYPE> D_STAR_DIST_MAP;
...
class AlgorithmDStar
{
	public:
		...
		static void CalculateDStarHomologyDistMap(const Map2D<T> &OriginalMap, const POS_2D &ZeroPos, const T &CutOffValue, D_STAR_DIST_MAP &DistMap);			// Calculates distance to zero pos from any point on the map
